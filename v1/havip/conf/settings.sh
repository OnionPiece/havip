...
HA_VRRP_ID="${OPENSHIFT_HA_VRRP_ID:-"0"}"
...
HA_CHECK_SCRIPT="${OPENSHIFT_HA_CHECK_SCRIPT:-"/var/lib/ipfailover/keepalived/app/check.py"}"
...
# The script must be accessible from inside the keepalived pod
HA_NOTIFY_SCRIPT="${OPENSHIFT_HA_NOTIFY_SCRIPT:-"/var/lib/ipfailover/keepalived/app/notify.py"}"

# The check script is run every HA_CHECK_INTERVAL seconds.
# Default is 2
HA_CHECK_INTERVAL="${OPENSHIFT_HA_CHECK_INTERVAL:-"2"}"

HA_CHECK_TIMEOUT="${OPENSHIFT_HA_CHECK_TIMEOUT:-"4"}"

HA_CHECK_RISE="${OPENSHIFT_HA_CHECK_RISE:-"3"}"

HA_CHECK_FALL="${OPENSHIFT_HA_CHECK_FALL:-"3"}"

HA_ADVERT_INT="${OPENSHIFT_HA_ADVERT_INT:-"2"}"
